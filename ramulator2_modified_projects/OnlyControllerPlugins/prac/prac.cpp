...
public:
    ...
    int get_activation_count(int bank_id, int row_addr) {
        if (bank_id < 0 || bank_id >= m_cfg.m_num_banks) {
            return 0; // Invalid bank
        }
        return m_bank_counters[bank_id].get_activation_count(row_addr);
    }
...
private:
    ...
    public: 
        ...
        uint32_t get_activation_count(int row_addr) const {
            auto it = m_counters.find(row_addr);
