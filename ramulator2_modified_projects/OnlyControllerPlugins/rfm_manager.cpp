...
public:
    void init() override { 
        ...
        m_raammt = param<int>("raammt").default_val(80); 
        m_raaimt = param<int>("raaimt").default_val(16);
    }
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
        ...
        register_stat(m_rfm_req_id).name("m_rfm_req_id");
    }
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override {
        ...
        if (m_clk % m_tREFI == 0) {
            for (int i = 0; i < m_bank_ctrs.size(); i++) {
                m_bank_ctrs[i] = std::max(0, m_bank_ctrs[i] - m_raaimt); 
            }
        }
        ...
        if (m_bank_ctrs[flat_bank_id] < m_raammt) {
            return;
        }
